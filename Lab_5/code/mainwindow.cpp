...
void MainWindow::paintEvent(QPaintEvent *)
{
   ...
   // draw original segments and polygon
    ...
    for(auto line:clipSegments[0])
        p.drawLine(pix.width()*(0.5+stp*line.first.x()),pix.height()*(0.5-stp*line.first.y()),pix.width()*(0.5+stp*line.second.x()),pix.height()*(0.5-stp*line.second.y()));
    p.setPen(QPen(Qt::red,3));
...
void MainWindow::on_selectDirectoryB_clicked()
{
    QFileDialog* dial = new QFileDialog(this);
      QString path = dial->getOpenFileName(this,tr("Choose file"),"/",tr("txt(*.txt)"));
      QFile file(path);
